...
LBFGS::LBFGS( std::shared_ptr<fresco::BasePotential> potential, const std::vector<double> x0,
        double tol, int M)
      ...
      step(x_.size())
{
    ...
    s_ = std::vector<double>(x_.size() * M_);
    y_ = std::vector<double>(x_.size() * M_);
}
...
void LBFGS::compute_lbfgs_step(std::vector<double>& step)
{
    if (k_ == 0){
...
double LBFGS::backtracking_linesearch(std::vector<double>& step)
{
    double fnew;
    ...
    rms_ = sqrt(gnorm) * inv_sqrt_size;
    return stepnorm * factor;
}

void LBFGS::reset(std::vector<double> &x0)
{
    if (x0.size() != x_.size()){
