#ifndef _FRESCO_OPTIMIZER_H__
#define _FRESCO_OPTIMIZER_H__
...
public:
     ...
     * accessors
     */
    inline virtual std::vector<double> get_x() const = 0;
    inline virtual std::vector<double> get_g() const = 0;
...
protected :
     ...
     */
    std::shared_ptr<fresco::BasePotential> potential_;
...
public :
    GradientOptimizer(std::shared_ptr<fresco::BasePotential> potential,
          const std::vector<double> x0, double tol=1e-4)
     ...
     */
    virtual void set_func_gradient(double f, std::vector<double>& grad)
    {
        ...
        rms_ = sqrt(gnorm/g_.size());
        func_initialized_ = true;
    }

    inline virtual void reset(std::vector<double> &x0)
    {
    ...
    inline std::vector<double> get_x() const { return x_; }
    inline std::vector<double> get_g() const { return g_; }
...
protected :
     ...
     */
    double compute_pot_norm(std::vector<double> & x)
    {
