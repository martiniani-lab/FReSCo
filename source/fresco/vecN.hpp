#ifndef FRESCO_ENT_VECN_H
#define FRESCO_ENT_VECN_H
...
public:
     ...
     */
    VecN(const std::vector<dtype> &x){std::copy(x.begin(), x.end(), this->begin());}
...
fresco::VecN<N, dtype> dot(MatrixNM<N, M, dtype> const & A, fresco::VecN<M, dtype> const & v)
{
    fresco::VecN<N, dtype> C(0);
    for (size_t i = 0; i<N; ++i){
        double val = 0;
...
template<size_t N, size_t M, typename dtype>
fresco::MatrixNM<M, N, dtype> transpose(MatrixNM<N, M, dtype> const & A)
{
    fresco::MatrixNM<M, N, dtype> mat;
    for (size_t i = 0; i<N; ++i){
        for (size_t k = 0; k<M; ++k){
            mat(k,i) = A(i,k);
        }
...
template<size_t N, typename dtype=double>
fresco::MatrixNM<N, N, dtype> identity()
{
    fresco::MatrixNM<N, N, dtype> A(0.);
    for (size_t i = 0; i<N; ++i) {
        A(i,i) = 1.;
    }
    return A;
